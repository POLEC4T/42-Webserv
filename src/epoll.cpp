...
/*   Updated: 2025/10/30 11:02:03 by mazakov          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int queueResponse(Client &client, std::string &response, int epollfd) {
	...
	if (my_epoll_ctl(epollfd, EPOLL_CTL_MOD, EPOLLIN | EPOLLOUT | EPOLLET,
					client.getFd()) == EXIT_FAILURE)
		return (EXIT_FAILURE);
...
int createSocket(const Server &server) {
	...
	int gai_ret = getaddrinfo(server.getHost().c_str(),
							server.getPorts()[0].c_str(), &hints, &addrinfos);
	if (gai_ret != 0) {
...
int addClient(Server &server, int servfd, int epollfd) {
	...
	if (my_epoll_ctl(epollfd, EPOLL_CTL_ADD, EPOLLIN | EPOLLET, clientfd) ==
		-1) {
		return (EXIT_FAILURE);
	}
...
int handleClientIn(Server &server, Client &client, int epollfd) {
	...
	try {
		...
		response = Response("HTTP/1.1", server.getErrorPageByCode(re.getCode()))
					.build();
		...
		for (int i = 0; i < eventsReady; ++i) {
			if (events[i].data.fd == servfd) {
				...
				if (events[i].events & EPOLLOUT) {
					std::cout
						<< "Sending response to client fd: " << client.getFd()
						<< std::endl;
