...
/*   Updated: 2025/11/04 14:41:12 by mniemaz          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
std::string getLocation(std::string &request) {
...
std::string getHTMLPageResponse() {
	...
	content << htmlFile.rdbuf();
	std::string html = (content.str());
	std::string response =
		"HTTP/1.1 200 OK\r\n"
		...
		"Content-Length: " + my_to_string(html.size()) + "\r\n"
		"\r\n" + html;
	return response;
}

std::string getChatResponse(std::string &request) {
	std::string body;
	size_t pos = request.find("\r\n\r\n");
	if (pos != std::string::npos) {
	...
	std::string response =
		...
		"Content-Length: " + my_to_string(body.size()) + "\r\n"
		"\r\n" + body;
	return response;
}
...
std::string getResponse(std::string &requestContent) {
	std::string response;
	std::string loc = getLocation(requestContent);
	std::string method = getMethod(requestContent);
	
	std::cout << "Method: " << method << ", Location: " << loc << std::endl;
	
	if (method == "OPTIONS") {
		response = getOptionsResponse();
	}
	else if (loc == "/" || loc == "/favicon.ico") {
		try {
			response = getHTMLPageResponse();
		} catch (...) {
			throw std::exception();
		}
	} else if (loc == "/message") {
		response = getChatResponse(requestContent);
	} else {
